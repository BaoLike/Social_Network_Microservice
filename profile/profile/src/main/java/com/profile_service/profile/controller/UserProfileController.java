...
import com.profile_service.profile.dto.request.PublicKeyUpdateRequest;
...
public class UserProfileController {
    ...
    ApiResponse<UserProfileResponse> updatePublicKey(@RequestBody @Valid PublicKeyUpdateRequest request) {
        return ApiResponse.<UserProfileResponse>builder()
                .message("Public key updated")
                .result(userProfileService.updatePublicKey(request.getPublicKey()))
                .build();
    }

    @GetMapping("/publicKey/{userId}")
    ApiResponse<UserProfileResponse> getPublicKey(@PathVariable String userId) {
        return ApiResponse.<UserProfileResponse>builder()
                .message("Public key fetched")
                .result(userProfileService.getPublicKeyByUserId(userId))
                .build();
    }
